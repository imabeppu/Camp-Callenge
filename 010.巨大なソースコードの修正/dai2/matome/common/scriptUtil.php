...
/**
 * idをhiddenでＰＯＳＴ先のページに渡す
 * @param type
 * @return type　idの情報を渡すhiddenのフォーム
 */
function hiddenID(){
  global $result;
echo  '<input type="hidden" name="id" value='.$result[0]['userID'].'>';
}

/**
 * modeをhiddenでＰＯＳＴ先のページに渡す
 * @param type
 * @return type　詳細画面に戻る時にmodeを伝えるhiddenのフォーム
 */
function reresult(){
echo  '<input type="hidden" name="mode" value="return">';
}
